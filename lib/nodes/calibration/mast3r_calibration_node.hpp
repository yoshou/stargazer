/// @file mast3r_calibration_node.hpp
/// @brief MASt3R-based multi-camera intrinsic and extrinsic calibration node.
...
/// @brief Multi-camera intrinsic and extrinsic calibration using the MASt3R model.
/// @details Collects image frames from all connected cameras and, on receipt of an
///          @b "estimate" trigger, runs MASt3R bundle-adjustment calibration in a
...
///
/// @par Properties
/// - `model_path` (`std::string`, default `""`) — path to the MASt3R ONNX model file
///
class mast3r_calibration_node : public coalsack::graph_node {
  ...
  std::unique_ptr<mast3r::mast3r_inference> engine_;

  graph_edge_ptr output_;

 public:
  mast3r_calibration_node()
      : coalsack::graph_node(), output_(std::make_shared<coalsack::graph_edge>(this)) {
    set_output(output_);
  }

  virtual ~mast3r_calibration_node() = default;

  virtual std::string get_proc_name() const override { return "mast3r_calibration"; }
  ...
  virtual void process(std::string input_name, coalsack::graph_message_ptr message) override {
    if (input_name == "estimate") {
      armed_.store(true);
      spdlog::info("mast3r_calibration_node: armed, waiting for next frame");
      return;
    }
    ...
    if (N < 2 || N < expected) {
      spdlog::info("mast3r_calibration_node: waiting for cameras ({}/{} ready), skipping frame", N,
                   expected);
      return;
    }
    ...
    armed_.store(false);
    spdlog::info("mast3r_calibration_node: processing {} cameras ({}x{})", N, orig_W, orig_H);

    // Preprocess: resize only, no undistortion
    std::vector<std::vector<float>> preprocessed(N);
    for (int i = 0; i < N; ++i) {
      preprocessed[i] = mast3r::preprocess_image(bgr_images[i]);
    }

    {
      ...
      if (!engine_) {
        if (model_path_.empty()) {
          spdlog::error("mast3r_calibration_node: model_path not set");
          return;
        }
        spdlog::info("mast3r_calibration_node: loading model from {}", model_path_);
        engine_ = std::make_unique<mast3r::mast3r_inference>(model_path_);
        spdlog::info("mast3r_calibration_node: model loaded");
      }
    }
    ...
    try {
      ...
      spdlog::error("mast3r_calibration_node: alignment failed: {}", ex.what());
      return;
    }
    ...
    if (ba_points.empty()) {
      spdlog::error("mast3r_calibration_node: no shared points after filtering, aborting");
      return;
    }
    ...
    try {
      ...
      spdlog::error("mast3r_calibration_node: BA failed: {}", ex.what());
      return;
    }

    spdlog::info("mast3r_calibration_node: BA done, RMSE {} → {} pixels", rmse_before, rmse_after);

    if (cam_results.empty()) {
      spdlog::error("mast3r_calibration_node: BA returned no cameras");
      return;
    }
    ...
    spdlog::info("mast3r_calibration_node: emitted calibration for {} cameras", N);
  }
};

}  // namespace stargazer

COALSACK_REGISTER_NODE(stargazer::mast3r_calibration_node, coalsack::graph_node)
