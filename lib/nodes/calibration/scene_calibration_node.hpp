 ...
 public:
  ...
  void push_frame(const std::map<std::string, cv::Mat>& frame) {
    ...
    for (const auto& [name, image] : frame) {
      ...
      detect_aruco_marker_for_scene(image, marker_corners, marker_ids);
...
class scene_calibration_node : public graph_node {
  ...
  scene_reconstruction reconstructor;

  graph_edge_ptr output;

 public:
  scene_calibration_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }

  virtual std::string get_proc_name() const override { return "scene_calibration"; }
...
COALSACK_REGISTER_NODE(stargazer::scene_calibration_node, coalsack::graph_node)
