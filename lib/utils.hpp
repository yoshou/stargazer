...
inline float distance_line_point(glm::vec3 line, glm::vec2 point) {
...
inline glm::vec2 project(const camera_t& camera, const glm::vec3& pt) {
  ...
  const auto x_ =
      x * (1 + k1 * r2 + k2 * r2 * r2 + k3 * r2 * r2 * r2) + 2 * p1 * x * y + p2 * (r2 + 2 * x * x);
  const auto y_ =
      y * (1 + k1 * r2 + k2 * r2 * r2 + k3 * r2 * r2 * r2) + 2 * p2 * x * y + p1 * (r2 + 2 * y * y);
  const auto proj_pt = proj_mat * (glm::vec3(x_, y_, 1.0f) / view_pt.w);
  const auto observed_pt = glm::vec2(proj_pt / proj_pt.z);

  if (observed_pt.x < 0 || observed_pt.x >= camera.width || observed_pt.y < 0 ||
      observed_pt.y >= camera.height) {
    return glm::vec2(-1);
  }
