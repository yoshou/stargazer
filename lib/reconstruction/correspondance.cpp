...
void compute_observations(const adj_list_t &adj,
  ...
  for (std::size_t v = 0; v < n; v++) {
    if (!visited[v]) {
      ...
      dfs(adj, v, visited, [&](std::size_t u) { connected_nodes.push_back(u); });
...
void remove_ambiguous_observations(const std::vector<node_t> &nodes, adj_list_t &adj,
  ...
  for (const auto &g : connected_graphs) {
    ...
    for (const auto &e : remove_edges) {
      ...
      {
        auto iter =
            std::remove_if(adj[v].begin(), adj[v].end(), [u](const size_t u_) { return u == u_; });
        adj[v].erase(iter, adj[v].end());
      }
      {
        auto iter =
            std::remove_if(adj[u].begin(), adj[u].end(), [v](const size_t v_) { return v == v_; });
...
struct point_cloud_2d {
 ...
 private:
  typedef nanoflann::KDTreeSingleIndexAdaptor<nanoflann::L2_Simple_Adaptor<float, point_cloud_2d>,
                                              point_cloud_2d, 2, /* dim */
                                              index_type>
      kd_tree_t;
 ...
 public:
  ...
  void build_index() { index->buildIndex(); }

  inline point_type operator[](std::size_t index) const { return points[index]; }
...
static glm::vec2 epipoloar_transfer(const camera_t &c3, const glm::mat3 &f1, const glm::mat3 &f2,
  ...
  const glm::vec2 pt3((line1.y * line2.z - line2.y * line1.z) / num,
...
void find_correspondance(const std::vector<std::vector<glm::vec2>> &pts,
  ...
  for (std::size_t i = 0; i < pts.size(); i++) {
    ...
    for (std::size_t j = 0; j < pts[i].size(); j++) {
      nodes.push_back(node_t{pts[i][j], i, j});
    }
  }
  ...
  for (auto [c1, c2] : camera_pairs) {
    ...
    for (std::size_t i = 0; i < pts[c1].size(); i++) {
      ...
      for (std::size_t j = 0; j < pts[c2].size(); j++) {
        ...
        // const auto marker = triangulate_undistorted(undist_norm_pts[c1][i],
        // undist_norm_pts[c2][j], cameras[c1], cameras[c2]);
        ...
        for (size_t c3 = 0; c3 < cameras.size(); c3++) {
          ...
          const auto pt = epipoloar_transfer(cameras[c3], f_mats[c1][c3], f_mats[c2][c3],
                                             undist_pts[c1][i], undist_pts[c2][j]);
...
#else
          float nearest_pt_dist_sq = std::numeric_limits<float>::max();
          const auto nearest_pt_idx =
              find_nearest_point(pt, undist_pts[c3], screen_thresh, nearest_pt_dist_sq);
#endif
