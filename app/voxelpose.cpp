...
voxelpose::voxelpose()
    : inference_heatmap(), inference_proposal(), inference_pose(), global_proj(new voxel_projector()), local_proj(new voxel_projector()), prop(new get_proposal()), joint_extract(new joint_extractor()),
...
std::array<float, 3> voxelpose::get_grid_size() const
{
    return grid_size;
}
std::array<float, 3> voxelpose::get_grid_center() const
{
    return grid_center;
}
void voxelpose::set_grid_size(const std::array<float, 3> &value)
{
    grid_size = value;
}
void voxelpose::set_grid_center(const std::array<float, 3> &value)
{
    grid_center = value;
}

std::vector<glm::vec3> voxelpose::inference(const std::vector<cv::Mat> &images_list, const std::vector<camera_data> &cameras_list)
{
...
const float *voxelpose::get_heatmaps() const
{
    return inference_heatmap->get_heatmaps();
}

void voxelpose::copy_heatmap_to(size_t num_views, float *data) const
{
    const auto heatmap_size = get_heatmap_width() * get_heatmap_height() * get_num_joints() * num_views;
    CUDA_SAFE_CALL(cudaMemcpy(data, get_heatmaps(), heatmap_size * sizeof(float), cudaMemcpyDeviceToHost));
}
uint32_t voxelpose::get_heatmap_width() const
{
    return inference_heatmap->get_heatmap_width();
}
uint32_t voxelpose::get_heatmap_height() const
{
    return inference_heatmap->get_heatmap_height();
}
uint32_t voxelpose::get_num_joints() const
{
    return 15;
}