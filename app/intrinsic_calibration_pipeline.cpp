...
void intrinsic_calibration_pipeline::run(const std::vector<node_def>& nodes) {
  pimpl->deploy(nodes);
}

void intrinsic_calibration_pipeline::start() { pimpl->run(); }

void intrinsic_calibration_pipeline::pause() { pimpl->stop_streaming(); }

void intrinsic_calibration_pipeline::stop() { pimpl->finalize(); }
