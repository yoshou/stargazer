...
class task_queue_processor {
  ...
  struct function_wrapper {
    Func func;
    function_wrapper(Func &&func) : func(std::move(func)) {}

    auto operator()() { return func(); }
  };

 public:
  task_queue_processor(size_t thread_count = 4) : workers(thread_count) {}
  ...
  void push(Task task) {
    ...
    }
    {
      ...
      auto callback = [p = std::move(p), task]() mutable { p.set_value(task()); };

      auto copyable_callback =
          std::make_shared<function_wrapper<decltype(callback)>>(std::move(callback));

      workers.push_task([copyable_callback]() { (*copyable_callback)(); });
#else
  ...
  bool pop(T &result) {
    ...
    results_cv.wait(lock, [this] { return !running || results.size() > 0; });
...
class mvpose_reconstruction : public multiview_image_reconstruction {
  ...
  mvpose_reconstruct(const std::map<std::string, stargazer::camera_t> &cameras,
                     const std::map<std::string, cv::Mat> &frame, glm::mat4 axis);
