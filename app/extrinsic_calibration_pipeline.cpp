...
class extrinsic_calibration_pipeline::impl {
  ...
  std::shared_ptr<stargazer::extrinsic_calibration_node> calib_node;
 ...
 public:
  ...
  void deploy(const std::vector<node_def>& nodes) {
    ...
    for (const auto& [name, node_ptr] : node_map) {
      ...
      if (!calib_node) {
        if (auto calib =
                std::dynamic_pointer_cast<stargazer::extrinsic_calibration_node>(node_ptr)) {
          calib_node = calib;
        }
      }
    }
    ...
    if (!calib_node) {
      spdlog::warn("Extrinsic calibration node not found");
    }
...
void extrinsic_calibration_pipeline::run(const std::vector<node_def>& nodes) {
  pimpl->deploy(nodes);
}

void extrinsic_calibration_pipeline::start() { pimpl->run(); }

void extrinsic_calibration_pipeline::pause() { pimpl->stop_streaming(); }

void extrinsic_calibration_pipeline::stop() { pimpl->finalize(); }

void extrinsic_calibration_pipeline::enable_marker_collecting(const std::string& name) {
  pimpl->enable_marker_collecting(name);
}

void extrinsic_calibration_pipeline::disable_marker_collecting(const std::string& name) {
